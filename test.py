...
def get_eligible_cutoffs(cursor, user_category, user_minority_list, user_home_university,
    ...
    SELECT
        college_name, branch_name, reservation_category, percentile, sorting_value, city
    FROM RankedChoices
    WHERE rn = 1
...
if __name__ == "__main__":
    conn = sqlite3.connect("CETrankDB.db")
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()

    print("Running test execution of get_eligible_cutoffs...\n")
    results = get_eligible_cutoffs(
        cursor=cursor,
        user_category="OPEN",
        user_minority_list=["Hindi", "Christian"],
        user_home_university="Rashtrasant Tukadoji Maharaj Nagpur University",
        gender="Male",
        cities=["Nagpur"],
        divisions=["Nagpur Division"],
        min_percentile_cet=82.0,
        max_percentile_cet=92.0,
        min_percentile_ai=60.0,
        max_percentile_ai=80.0,
        is_tech=True,
        is_ews=True
    )
    ...
    print("All results successfully saved to test.json\n")

    for i, row in enumerate(results[:5]):  # Print top 5 for readability
